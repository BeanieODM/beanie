...
async def test_distinct_find_one():
    await Sample(
        timestamp=datetime.datetime.now(tz=datetime.timezone.utc),
        increment=0,
        integer=10,
        ...
        nested=Nested(
            integer=1,
            option_1=Option1(s="T"),
            union=Option1(s="T"),
        ),
        union=Option1(s="T"),
        geo=GeoObject(coordinates=[0.0, 0.0]),
    ).insert()
    await Sample(
        timestamp=datetime.datetime.now(tz=datetime.timezone.utc),
        increment=1,
    ...
    # FindOne.distinct delegates to FindQuery.distinct
    values = await Sample.find_one(Sample.integer == 10).distinct("string")
    assert sorted(values) == ["one", "two"]
