...
class TestForwardReferences:
    ...
    async def test_forward_references_basic(self):
        ...
        post = PostForwardRef(title="Test Post", content="Test content", author=user)
        await post.insert()
        ...
        fetched_post = await PostForwardRef.find_one(PostForwardRef.id == post.id, fetch_links=True)
    ...
    async def test_forward_references_optional(self):
        ...
        profile = ProfileForwardRef(user=user, favorite_post=post, bio="Test bio")
    ...
    async def test_forward_references_cross_module_simulation(self):
        ...
        comment = CommentForwardRef(post=post, author=user, content="Great post!")
    ...
    async def test_forward_references_list_links(self):
        ...
        comment1 = CommentForwardRef(post=post, author=user, content="First comment")
        comment2 = CommentForwardRef(post=post, author=user, content="Second comment")
        ...
        assert all(isinstance(comment, CommentForwardRef) for comment in fetched_post.comments)
    ...
    async def test_forward_reference_proxy_functionality(self):
        ...
        with pytest.raises(ValueError, match="Forward reference 'NonExistentClass' could not be resolved"):
            proxy.resolve()
    ...
    async def test_forward_references_work_after_registry_clear(self):
        ...
        post = PostForwardRef(title="Test Post 2", content="Test content 2", author=user)
        await post.insert()

        # Verify forward references still work
        fetched_post = await PostForwardRef.find_one(PostForwardRef.author.id == user.id, fetch_links=True)
        assert fetched_post is not None
        assert isinstance(fetched_post.author, UserForwardRef)
    ...
    async def test_bidirectional_forward_references(self):
        ...
        user = UserForwardRef(username="bidirectional_user", email="bi@example.com")
        await user.insert()

        post = PostForwardRef(title="Bidirectional Post", content="Content", author=user)
        await post.insert()
        ...
        assert fetched_profile.favorite_post.author.id == fetched_profile.user.id
    ...
    async def test_github_issue_1035_exact_scenario(self):
        ...
        user = UserForwardRef(username="github_user", email="github@example.com")
        await user.insert()
        ...
        assert isinstance(fetched_comment.author, UserForwardRef)  # User forward ref works
        assert isinstance(fetched_comment.post, PostForwardRef)    # Post link works
        assert isinstance(fetched_comment.post.author, UserForwardRef)  # Nested forward ref works
