...
async def test_append_operation_generates_push_with_exact_values(
    ...
    for cmd_name, command in logger.commands:
        # Check in findAndModify commands which is what Beanie uses internally
        if cmd_name == "findAndModify" and "update" in command:
...
async def test_multiple_list_operations_with_exact_values(
    db, create_command_logger
):
...
async def test_nested_list_updates_use_full_update(db, create_command_logger):
...
async def test_concurrent_updates_to_list(db, create_command_logger):
    """Test that concurrent updates to lists are correctly saved"""

    create_command_logger({"update", "findAndModify"})
    await init_beanie(
        database=db, document_models=[ComplexListOperationsModel]
    )
...
async def test_state_management_replace_objects_behavior(
    db, create_command_logger
):
