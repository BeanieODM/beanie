...
async def test_migration_free_fall(settings, notes, db):
    if not (await db.client.is_mongos) and not len(db.client.nodes) > 1:
        return pytest.skip(
            "MongoDB server does not support transactions as it is neither a mongos instance nor a replica set."
        )
