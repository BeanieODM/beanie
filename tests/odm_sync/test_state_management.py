...
def doc_default(state):
    return SyncDocumentWithTurnedOnStateManagement.parse_obj(state)


@pytest.fixture
def doc_replace(state):
    return SyncDocumentWithTurnedOnReplaceObjects.parse_obj(state)
...
def test_use_state_management_property():
    assert (
        SyncDocumentWithTurnedOnStateManagement.use_state_management() is True
    )
    assert (
        SyncDocumentWithTurnedOffStateManagement.use_state_management()
        is False
    )


def test_save_state():
    doc = SyncDocumentWithTurnedOnStateManagement(
...
def test_parse_object_with_saving_state():
    ...
    doc = SyncDocumentWithTurnedOnStateManagement.parse_obj(obj)
    assert doc.get_saved_state() == obj


def test_saved_state_needed():
    doc_1 = SyncDocumentWithTurnedOffStateManagement(num_1=1, num_2=2)
    with pytest.raises(StateManagementIsTurnedOff):
        doc_1.is_changed

    doc_2 = SyncDocumentWithTurnedOnStateManagement(
...
def test_find_one(saved_doc_default, state):
    ...
    new_doc = SyncDocumentWithTurnedOnStateManagement.find_one(
        SyncDocumentWithTurnedOnStateManagement.id == saved_doc_default.id
    ).run()
    assert new_doc.get_saved_state() == state
...
def test_find_many():
    ...
    SyncDocumentWithTurnedOnStateManagement.insert_many(docs)

    found_docs = SyncDocumentWithTurnedOnStateManagement.find(
        SyncDocumentWithTurnedOnStateManagement.num_1 > 4
    ).to_list()
...
def test_insert(state_without_id):
    doc = SyncDocumentWithTurnedOnStateManagement.parse_obj(state_without_id)
    assert doc.get_saved_state() is None
    doc.insert()
