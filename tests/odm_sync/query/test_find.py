...
def test_bson_encoders_filed_types():
    custom = SyncDocumentWithBsonEncodersFiledsTypes(
        color="7fffd4", timestamp=datetime.datetime.utcnow()
    )
    print(SyncDocumentWithBsonEncodersFiledsTypes.get_settings())
    print(custom)
    c = custom.insert()
    c_fromdb = SyncDocumentWithBsonEncodersFiledsTypes.find_one(
        SyncDocumentWithBsonEncodersFiledsTypes.color == Color("7fffd4")
    ).run()
