from beanie.odm_sync import PydanticObjectId
from tests.odm_sync.models import SyncDocumentTestModel, Sample
...
class TestRun:
    ...
    def test_find_many_skip(self, preset_documents):
        ...
        result = ~Sample.find_many(Sample.increment > 2).find_many(
            Sample.nested.optional == None).skip(1)
        assert len(result) == 3
    ...
    def test_update_many(self, documents):
        documents(10, "foo")
        documents(7, "bar")
        ~SyncDocumentTestModel.find_many({"test_str": "foo"}).update(
            {"$set": {"test_str": "bar"}}
        )
        bar_documetns = ~SyncDocumentTestModel.find_many(
            {"test_str": "bar"}
        )
        assert len(bar_documetns) == 17
        foo_documetns = ~SyncDocumentTestModel.find_many(
            {"test_str": "foo"}
        )
    ...
    def test_update_all(self, documents):
        documents(10, "foo")
        documents(7, "bar")
        ~SyncDocumentTestModel.update_all(
        ...
        assert len(foo_documetns) == 0
        smth_else_documetns = ~SyncDocumentTestModel.find_many(
            {"test_str": "smth_else"}
        )
    ...
    def test_delete_all(self, documents):
        documents(4, "uno")
        documents(2, "dos")
        documents(1, "cuatro")
