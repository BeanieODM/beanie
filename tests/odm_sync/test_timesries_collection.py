...
from tests.odm_sync.models import SyncDocumentWithTimeseries
...
def test_timeseries_collection(db):
    ...
    if major_version < 5:
        with pytest.raises(MongoDBVersionError):
            init_beanie(
                database=db, document_models=[SyncDocumentWithTimeseries]
            )

    if major_version >= 5:
        init_beanie(database=db, document_models=[SyncDocumentWithTimeseries])
        info = db.command(
            {
                "listCollections": 1,
