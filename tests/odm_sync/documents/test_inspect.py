from beanie.odm_sync.models import InspectionStatuses
...
def test_inspect_ok(documents):
    documents(10, "smth")
    result = SyncDocumentTestModel.inspect_collection()
...
def test_inspect_fail(documents):
    documents(10, "smth")
    result = SyncDocumentTestModelFailInspection.inspect_collection()
    assert result.status == InspectionStatuses.FAIL
    assert len(result.errors) == 10
    assert (
        "1 validation error for SyncDocumentTestModelFailInspection"
...
def test_inspect_ok_with_session(documents, session):
    documents(10, "smth")
    result = SyncDocumentTestModel.inspect_collection(session=session)
    assert result.status == InspectionStatuses.OK
    assert result.errors == []
