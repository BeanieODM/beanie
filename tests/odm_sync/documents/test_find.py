...
def test_find_one(documents):
    ...
    new_document = SyncDocumentTestModel.find_one({"test_str": "kipasa"}).run()
...
def test_find_one_not_found(documents):
    ...
    new_document = SyncDocumentTestModel.find_one({"test_str": "wrong"}).run()
    assert new_document is None
...
def test_find_one_more_than_one_found(documents):
    documents(10, "one")
    documents(10, "two")
    new_document = SyncDocumentTestModel.find_one({"test_str": "one"}).run()
...
def test_find_all_limit(documents):
    ...
    documents(2, "dos")
    documents(1, "cuatro")
    result = SyncDocumentTestModel.find_all(limit=5).to_list()
    assert len(result) == 5
...
def test_find_all_skip(documents):
    ...
    documents(2, "dos")
    documents(1, "cuatro")
    result = SyncDocumentTestModel.find_all(skip=1).to_list()
    assert len(result) == 6
...
def test_find_all_sort(documents):
    ...
    result = SyncDocumentTestModel.find_all(
        sort=[
...
def test_find_many(documents):
    ...
    documents(2, "dos")
    documents(1, "cuatro")
    result = SyncDocumentTestModel.find_many(
        SyncDocumentTestModel.test_str == "uno"
    ).to_list()
    assert len(result) == 4
...
def test_find_many_not_found(documents):
    ...
    documents(2, "dos")
    documents(1, "cuatro")
    result = SyncDocumentTestModel.find_many({"test_str": "wrong"}).to_list()
    assert len(result) == 0
...
def test_find_one_with_session(documents, session):
    ...
    new_document = SyncDocumentTestModel.find_one(
...
def test_find_all_with_session(documents, session):
    ...
    documents(2, "dos")
    documents(1, "cuatro")
    result = SyncDocumentTestModel.find_all(session=session).to_list()
    assert len(result) == 7
