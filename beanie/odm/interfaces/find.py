...
FindType = TypeVar(
    "FindType", bound=Union["Document", "UnionDoc", "View"]
)  # only used as type hinting! UnionDoc inherit from FindInterface. UnionDoc doesnt inherit from LazyParse so its not compatible with BaseModel would need to inherit LazyParse to UnionDoc if we wanna fix errors in the case we import UnionDoc which I didnt
